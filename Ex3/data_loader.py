    ...
    def _build_dataset(self, sentences):
        ...
        for sent, sent_pointers in zip(sentences, tree_pointers):
            try:
                ...
                # [Aran]: create dictionary for children of node, order them by running index.
                #       Every entry is a list of nodes
                children_dict = {i: [] for i in range(len(sent_pointers))}

                for i, p in enumerate(sent_pointers):
                    # [Aran]:if index of pointer exists in the sentence -> create node form that word
                    if i < len(sent):
                        node_text = [sent[i]]

                        # [Aran]: create node based on that word.
                        node = SentimentTreeNode(text=node_text, sentiment_val=get_val_from_phrase(node_text),
                                                 min_token_idx=i)

                    # [Aran]: if the index is not in the sentence ->
                    else:
                        # [Aran]: take all children of current index.
                        children = children_dict[i]

                        # [Aran]: sort children in that entry according to min token index.
                        children = sorted(children, key= lambda n: n.min_token_idx)

                        # [Aran]: create node_text for union of all node's children's texts.
                        node_text = []
                        for child in children:
                            node_text.extend(child.text)

                        # [Aran]: create new node from union text.
                        node = SentimentTreeNode(text=node_text, sentiment_val=get_val_from_phrase(node_text),
                                                 children=children, min_token_idx=children[0].min_token_idx)

                        # [Aran]: the parent of all children in the children dict is the node.
                        for child in children:
                            child.parent = node

                    # [Aran]: add entry to children dictionary: append node to previous child list.
                    if p > 0:
                        children_dict[p - 1].append(node)
                    last_node = node

                # [Aran]: create sentence object from last node in the current sentence.
                new_sentence = Sentence(last_node)

                # [Aran]: append last node sentence to the labeled sentences list, if it is a charged node.
        ...
        if sent.sentiment_class == NEUTRAL_SENTIMENT:
            ...
            for child in sent.root.children:
                if child.sentiment_class == 1 - root_polarity:
                    # FIXME: is this a mistake? shouldn't it be child_sentiment_class == -(root_polarity)
                    #       as in 1 == -(-1)
                    return True
            return False
...
def get_sentiment_words(sent: Sentence):
    """
    Returns (leaf) children of given sentence, which match the polarity of the given sentence.
    :param sent:
    :return:
    """
